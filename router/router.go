...
func NewRouter(logger types.Logger, sk ed25519.PrivateKey, opts ...RouterOption) *Router {
	...
	_, insecure := os.LookupEnv("XPROTO_DISABLE_SIGNATURES")
	r := &Router{
		log:           logger,
		context:       ctx,
		cancel:        cancel,
...
// Close will stop the Xproto node. Once this has been called, the node cannot
...
// given port number on the Xproto node. The peering will
